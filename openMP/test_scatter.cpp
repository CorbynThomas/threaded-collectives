...
* FILE: test.cpp
* DESCRIPTION:
*   unit tests for both gpu aware scatter and gather functions
...
int main(int argc, char** argv)
{
    ...
    #pragma omp parallel num_threads(3)
    {
        ...
        if(rank == MASTER)
        {
            sendbuf.resize(sendcount * size, -1);
            for(int i = 0; i < sendbuf.size(); ++i)
            {
                sendbuf[i] = (tid * 1000) + i;
            }
        }
        ...
        if(rank == 0)
        {
            ...
            for (int t = 0; t < num_threads; ++t)
            {
                ...
                if (tid == t)
                {
                    std::cout << "Rank " << rank << ", Thread " << tid << " values: ";
        ...
        int rc = thread_aware_scatter(thread_scatter, sendbuf.data(), sendcount, MPI_INT, recvbuf.data(), sendcount, MPI_INT, root, MPI_COMM_WORLD);
        ...
        for (int r = 0; r < size; ++r)
        {
            for (int t = 0; t < num_threads; ++t)
            {
                ...
                #pragma omp master
                    MPI_Barrier(MPI_COMM_WORLD);
                if(rank == r && tid == t)
                    std::cout << "Rank " << r << ", Thread " << t << " value: " << recvbuf[0] << ::endl;
            }
            #pragma omp barrier
            #pragma omp master
                MPI_Barrier(MPI_COMM_WORLD);
