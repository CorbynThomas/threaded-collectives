    ...
    #pragma omp parallel num_threads(12)
    {
        ...
        for (int r = 0; r < size; ++r)
        {
            for (int t = 0; t < num_threads; ++t)
            {
                ...
                if(rank == r && tid == t)
                    std::cout << "Rank " << r << ", Thread " << t << " value: " << buf[0] << " " << buf[1] << std::endl;
            }
            best_barrier();
        ...
        int rc = thread_aware_reduce(thread_reduce, buf.data(), recbuf.data(), sendcount, MPI_INT, MPI_SUM, MASTER, MPI_COMM_WORLD);
        ...
        if(rank == MASTER)
        {
            ...
            for (int t = 0; t < num_threads; ++t)
            {
                ...
                if (tid == t)
                {
                    std::cout << "Rank " << rank << ", Thread " << tid << " values: " << recbuf[0] << " " << recbuf[1] << endl;
                }
            }
        }
